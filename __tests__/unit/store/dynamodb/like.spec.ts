      ...
      describe('like', function () {
        describe('postByUserLike', function () {
          ...
          it('postsByUserLike(User(a)) === 0', async () => {
            const {items} = await postsByUserLike(opDdb, {userId, entity})
            expect(items.length).toEqual(0)
          })
          it('User(a)가 Post(0) 에 공감', async () => {
            const like = createLike({
              data: {entity, targetId, createdAt, data},
              user,
            })
            const added = await addLike(opDdb, {data: like})
            expect(like).toEqual(added)
          })
          ...
          it('postsByUserLike(User(b)) === 1', async () => {
            const {items} = await postsByUserLike(opDdb, {userId: member_b.id, entity})
            expect(items.length).toEqual(1)
          })
          ...
          it('postsByUserLike(User(a)) === Comment(0): 엔티티 필터', async () => {
            const {items} = await postsByUserLike(opDdb, {userId, entity: EEntity.Comment})
            expect(items.length).toEqual(0)
          })
