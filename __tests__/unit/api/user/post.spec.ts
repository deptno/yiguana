  ...
  describe('api', () => {
    describe('user', () => {
      describe('post', () => {
        ...
        it('list(0)', async () => {
          const {items} = await yiguana.post.list({
            data: {},
          })
        ...
        it('create post, 회원 글쓰기', async () => {
          ...
          const {items} = await yiguana.user.post.list({
            user: member_a,
            data: {},
          })
        ...
        it('like post -> like post', async () => {
          ...
          const firstPost = await yiguana.post.read({data: post})
          expect(firstPost.likes).toEqual(2)
          const {items: before} = await yiguana.post.list({
            data: {},
          })
